pipeline {
    ...
    stages {
        ...
        stage("tomcat-build") {
            steps { withCredentials([sshUserPrivateKey(credentialsId: 'new_key', keyFileVariable: 'new_key', usernameVariable: 'ubuntu')]) {
                sh '''
                ssh -i ${new_key} -o StrictHostKeyChecking=no ubuntu@18.183.239.236<<EOF
